...
class ScrapingController extends Controller
{
    ...
    public function start(Request $request, ScrapingProgressService $progressService)
    {
        ...
        register_shutdown_function(function () use ($jobId, $activeSources, $noTruncate, $sourceCommands) {
            $progressService = app(\App\Services\ScrapingProgressService::class);
            $controller = new self();
            $controller->executeScraping($jobId, $activeSources, $noTruncate, $progressService, $sourceCommands);
        });
    ...
    /**
     * Exécute le scraping avec suivi de progression
     */
    public function executeScraping(
        ...
        try {
            ...
            foreach ($activeSources as $index => $source) {
                ...
                $processData = $this->startScraperProcess($command, $source);
                ...
                if ($progressService->isCancelled($jobId)) {
                    ...
                    foreach ($processes as $source => $processData) {
                        if (is_resource($processData['process'])) {
                            ...
                            if (isset($processData['pipes'][1])) fclose($processData['pipes'][1]);
                            if (isset($processData['pipes'][2])) fclose($processData['pipes'][2]);
     ...
     */
    private function startScraperProcess(string $command, string $source)
    {
        $phpPath = PHP_BINARY;
