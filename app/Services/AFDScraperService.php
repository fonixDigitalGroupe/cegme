...
class AFDScraperService
{
    ...
    private function scrapePage(int $page): array
    {
        ...
        try {
            ...
            foreach ($links as $link) {
                ...
                if (stripos($href, '/appels-a-projets/liste?') !== false || 
                ...
                if (!$isNavigation && stripos($href, 'appels-a-projets') !== false) {
                    ...
                    if (stripos($href, '/liste?') === false && stripos($href, '/liste#') === false && 
            ...
            foreach ($fallbackItems as $item) {
                ...
                if ($itemLinks->length > 0) {
                    ...
                    if (stripos($itemHref, '/liste?') !== false || 
            ...
            if (count($items) < 5) {
                ...
                foreach ($allDivs as $div) {
                    ...
                    if (preg_match('/\d{1,2}\s+(janvier|février|mars|avril|mai|juin|juillet|août|septembre|octobre|novembre|décembre)/i', $text) &&
            ...
            foreach ($allTextNodes as $textNode) {
                ...
                if (preg_match('/\b(\d+)\b/', $text, $matches)) {
                    ...
                    if ($possiblePage > $currentPage && $possiblePage <= 100) {
                        ...
                        if (stripos($parentClass, 'page') !== false || 
    ...
    private function extractCountry(\DOMElement $item, DOMXPath $xpath): ?string
    {
        ...
        $countries = [
            // Afrique
            'République centrafricaine', 'Tchad', 'Cameroun', 'Sénégal', 
            'Burkina Faso', 'Mali', 'Niger', 'Madagascar', 'Comores', 
            'Maurice', 'Burundi', 'Rwanda', 'Tanzanie', 'Kenya', 'Ouganda',
            'Afrique du Sud', 'Algérie', 'Angola', 'Bénin', 'Cap-Vert',
            'Congo', 'Côte d\'Ivoire', 'Djibouti', 'Égypte', 'Éthiopie',
            'Gabon', 'Gambie', 'Ghana', 'Guinée-Bissau', 'Libéria',
            'Malawi', 'Maroc', 'Mauritanie', 'Mozambique', 'Namibie',
            'Nigéria', 'République démocratique du Congo', 'Sao Tomé-et-Principe',
            'Sierra Leone', 'Soudan', 'Togo', 'Tunisie', 'Zambie', 'Zimbabwe',
            // Orients
            'Afghanistan', 'Azerbaïdjan', 'Arménie', 'Bangladesh', 'Birmanie',
            'Cambodge', 'Chine', 'Géorgie', 'Inde', 'Indonésie', 'Irak',
            'Jordanie', 'Kazakhstan', 'Laos', 'Liban', 'Malaisie', 'Moldavie',
            'Mongolie', 'Ouzbékistan', 'Pakistan', 'Palestine', 'Philippines',
            'République de Corée', 'Sri Lanka', 'Thaïlande', 'Turquie',
            'Ukraine', 'Vietnam', 'Balkans occidentaux',
            // Trois Océans
            'Fidji', 'Guadeloupe', 'Guyane', 'Haïti', 'La Réunion',
            'Martinique', 'Mayotte', 'Nouvelle-Calédonie', 'Papouasie-Nouvelle-Guinée',
            'Polynésie française', 'République dominicaine', 'Suriname',
            'Vanuatu', 'Wallis-et-Futuna',
            // Amérique latine
            'Argentine', 'Bolivie', 'Brésil', 'Colombie', 'Costa Rica',
            'Cuba', 'Équateur', 'Mexique', 'Pérou', 'Uruguay',
            // Europe
            'France', 'Belgique',
        ];
        ...
        $zones = [
            'Afrique', 'Afrique Centrale', 'Afrique de l\'Ouest', 'Afrique de l\'Est',
            'Afrique Australe', 'Afrique du Nord', 'Afrique subsaharienne',
            'Sous-région', 'Région', 'Zone', 'Bassin', 'Espace',
            'Amérique latine', 'Amérique centrale', 'Amérique du Sud',
            'Asie', 'Asie du Sud-Est', 'Asie centrale', 'Moyen-Orient',
            'Europe', 'Méditerranée', 'Caraïbes', 'Océan Indien',
            'Pacifique', 'Mondial', 'International',
        ];
