    ...
    <body class="bg-white text-[#1b1b18] min-h-screen" x-data="{ activeFilter: 'all', searchQuery: '' }" style="background-color: #ffffff !important;">
        ...
        <section class="w-full bg-white px-4 sm:px-6 lg:px-8" style="padding: 48px 0;">
            <div class="max-w-7xl mx-auto">
                <!-- Search Bar -->
                <div class="relative mb-8" style="margin-bottom: 32px;">
                ...
                <!-- Filter Buttons -->
                <div class="flex flex-wrap gap-3 justify-center" style="display: flex; flex-wrap: wrap; gap: 12px; justify-content: center;">
                    <button
                        @click="activeFilter = 'all'"
                        :class="activeFilter === 'all' ? 'bg-green-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'"
                        class="px-4 py-2 rounded-lg font-medium transition-colors"
                        style="padding: 8px 16px; border-radius: 6px; font-size: 14px; font-weight: 500;"
                    >
                        Tous
                    </button>
                    <button
                        @click="activeFilter = 'environnement'"
                        :class="activeFilter === 'environnement' ? 'bg-green-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'"
                        class="px-4 py-2 rounded-lg font-medium transition-colors"
                        style="padding: 8px 16px; border-radius: 6px; font-size: 14px; font-weight: 500;"
                    >
                        Environnement
                    ...
                    <button
                        @click="activeFilter = 'geologie'"
                        :class="activeFilter === 'geologie' ? 'bg-green-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'"
                        class="px-4 py-2 rounded-lg font-medium transition-colors"
                        style="padding: 8px 16px; border-radius: 6px; font-size: 14px; font-weight: 500;"
                    >
                        Géologie
                    ...
                    <button
                        @click="activeFilter = 'mines'"
                        :class="activeFilter === 'mines' ? 'bg-green-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'"
                        class="px-4 py-2 rounded-lg font-medium transition-colors"
                        style="padding: 8px 16px; border-radius: 6px; font-size: 14px; font-weight: 500;"
                    >
                        Mines
                    ...
                    <button
                        @click="activeFilter = 'actualites'"
                        :class="activeFilter === 'actualites' ? 'bg-green-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'"
                        class="px-4 py-2 rounded-lg font-medium transition-colors"
                        style="padding: 8px 16px; border-radius: 6px; font-size: 14px; font-weight: 500;"
                    >
                        Actualités
                    ...
                    <button
                        @click="activeFilter = 'projets'"
                        :class="activeFilter === 'projets' ? 'bg-green-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'"
                        class="px-4 py-2 rounded-lg font-medium transition-colors"
                        style="padding: 8px 16px; border-radius: 6px; font-size: 14px; font-weight: 500;"
                    >
                        Projets
                    ...
                    <button
                        @click="activeFilter = 'formation'"
                        :class="activeFilter === 'formation' ? 'bg-green-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'"
                        class="px-4 py-2 rounded-lg font-medium transition-colors"
                        style="padding: 8px 16px; border-radius: 6px; font-size: 14px; font-weight: 500;"
                    >
                        Formation
                        ...
                        @foreach($posts as $post)
                            <article class="bg-white rounded-2xl shadow-md overflow-hidden cursor-pointer hover:shadow-lg transition-shadow" style="background-color: rgb(255, 255, 255); border-radius: 24px; padding: 0px; box-shadow: rgba(0, 0, 0, 0.1) 0px 10px 15px -3px, rgba(0, 0, 0, 0.1) 0px 4px 6px -4px; cursor: pointer;" onclick="window.location.href='{{ route('blog.show', $post->slug) }}'">
                                ...
                                <div class="p-6" style="padding: 24px;">
                                    ...
                                    @endif
                                    <div class="flex items-center gap-4 mb-4 text-sm text-gray-500" style="display: flex; align-items: center; gap: 16px; margin-bottom: 16px; font-size: 14px; color: rgb(107, 114, 128);">
                                    ...
                                    @if($post->tags->count() > 0)
                                        <div class="flex flex-wrap gap-2" style="display: flex; flex-wrap: wrap; gap: 8px;">
                                            @foreach($post->tags->take(3) as $tag)
                                                <span class="px-3 py-1 bg-gray-100 text-gray-700 rounded-full text-xs font-medium" style="padding: 4px 12px; background-color: rgb(243, 244, 246); color: rgb(55, 65, 81); border-radius: 9999px; font-size: 12px; font-weight: 500;">
                                                    #{{ $tag->name }}
                                                </span>
                                            @endforeach
                                        </div>
