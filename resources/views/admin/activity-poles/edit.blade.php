...
<form method="POST" action="{{ route('admin.activity-poles.update', $activityPole) }}" style="background-color: #ffffff; padding: 1.5rem; border: 1px solid #d1d5db; border-radius: 4px;">
    @csrf
    @method('PUT')
    ...
    <div style="margin-bottom: 1.5rem;">
        ...
        <input type="text" id="name" name="name" value="{{ old('name', $activityPole->name) }}" required style="width: 100%; padding: 0.5rem 0.75rem; border: 1px solid #d1d5db; border-radius: 4px; font-size: 0.875rem;">
    </div>
    ...
    <div style="margin-bottom: 1.5rem;">
        ...
        <textarea id="description" name="description" rows="3" style="width: 100%; padding: 0.5rem 0.75rem; border: 1px solid #d1d5db; border-radius: 4px; font-size: 0.875rem;">{{ old('description', $activityPole->description) }}</textarea>
    </div>

    <div style="margin-bottom: 1.5rem;">
        <label style="display: block; font-size: 0.875rem; font-weight: 500; color: #374151; margin-bottom: 0.5rem;">Mots-clés</label>
        <div id="keywords-container">
            @foreach($activityPole->keywords as $keyword)
                <div style="display: flex; gap: 0.5rem; margin-bottom: 0.5rem;">
                    <input type="text" name="keywords[]" value="{{ $keyword->keyword }}" style="flex: 1; padding: 0.5rem 0.75rem; border: 1px solid #d1d5db; border-radius: 4px; font-size: 0.875rem;">
