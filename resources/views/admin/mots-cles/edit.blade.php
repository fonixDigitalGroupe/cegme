...
<div style="margin-bottom: 1.5rem;">
    <h1 style="font-size: 1.5rem; font-weight: 600; color: #1a1a1a; margin: 0; font-family: 'Inter', -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif;">Modifier un mot-clé</h1>
</div>

<form action="{{ route('admin.mots-cles.update', $motsCle) }}" method="POST">
    @csrf
    @method('PUT')
    ...
    <div style="background-color: #ffffff; border: 1px solid #d1d5db; border-radius: 4px; padding: 1.5rem;">
        <div style="margin-bottom: 1.5rem;">
            ...
            <input type="text" name="nom" id="nom" value="{{ old('nom', $motsCle->nom) }}" required class="form-input" placeholder="Ex: EIES, Impact environnemental, Audit environnemental..." style="width: 100%; padding: 0.625rem 0.875rem; border: 1px solid #d1d5db; border-radius: 4px; font-size: 0.875rem; font-family: 'Inter', -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif;">
        ...
        <div style="margin-bottom: 1.5rem;">
            <label class="form-label" style="font-size: 0.875rem; font-weight: 500; color: #374151; margin-bottom: 0.5rem; display: block; font-family: 'Inter', -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif;">Pôle d'Activité <span style="color: #dc2626;">*</span></label>
            <select name="pole_activite_id" id="pole_activite_id" required class="form-input" style="width: 100%; padding: 0.625rem 0.875rem; border: 1px solid #d1d5db; border-radius: 4px; font-size: 0.875rem; font-family: 'Inter', -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif;">
                <option value="">Sélectionner un pôle d'activité</option>
                @foreach($poleActivites as $poleActivite)
                    <option value="{{ $poleActivite->id }}" {{ old('pole_activite_id', $motsCle->pole_activite_id) == $poleActivite->id ? 'selected' : '' }}>{{ $poleActivite->nom }}</option>
                @endforeach
            </select>
