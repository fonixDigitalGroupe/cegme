...
<div class="card">
    <form action="{{ route('admin.posts.update', $post) }}" method="POST" enctype="multipart/form-data">
        ...
        <div style="margin-bottom: 1.25rem;">
            ...
            <textarea name="excerpt" rows="3" class="form-input">{{ old('excerpt', $post->excerpt) }}</textarea>
        ...
        <div style="margin-bottom: 1.25rem;">
            ...
            @if($post->featured_image)
                <div style="margin-bottom: 0.5rem;">
                    <img src="{{ asset('storage/' . $post->featured_image) }}" alt="Image actuelle" style="height: 128px; width: auto; border-radius: 0.375rem;">
                </div>
            @endif
            ...
            <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(200px, 1fr)); gap: 0.75rem;">
                @foreach($tags as $tag)
                    <label style="display: flex; align-items: center; cursor: pointer;">
                        <input type="checkbox" name="tags[]" value="{{ $tag->id }}" {{ in_array($tag->id, old('tags', $post->tags->pluck('id')->toArray())) ? 'checked' : '' }} style="margin-right: 0.5rem;">
                        <span>{{ $tag->name }}</span>
                    </label>
                @endforeach
            </div>
