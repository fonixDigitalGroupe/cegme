...
<div style="overflow-x: auto; background-color: #ffffff; border: 1px solid #d1d5db; border-radius: 4px;">
    <table style="width: 100%; border-collapse: collapse; margin: 0;">
        ...
        <tbody>
            ...
            <tr style="border-bottom: 1px solid #e5e7eb;">
                ...
                <td style="padding: 0.75rem 1rem; text-align: right;">
                    <div style="display: flex; align-items: center; justify-content: flex-end; gap: 0.375rem;">
                        ...
                        <form action="{{ route('admin.pole-activites.destroy', $poleActivite) }}" method="POST" style="display: inline; margin: 0;" onsubmit="return confirm('Êtes-vous sûr de vouloir supprimer ce pôle d\'activité ?');">
                            @csrf
                            @method('DELETE')
            ...
            @empty
            <tr>
                <td colspan="3" style="text-align: center; padding: 2rem; color: #9ca3af; font-size: 0.875rem; font-family: 'Inter', -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif;">Aucun pôle d'activité trouvé</td>
            </tr>
            @endforelse
        </tbody>
    </table>
...
<div style="margin-top: 0; padding: 0.75rem 1rem; background-color: #ffffff; border: 1px solid #d1d5db; border-top: none; border-radius: 0 0 4px 4px; display: flex; justify-content: space-between; align-items: center;">
    <div style="font-size: 0.875rem; color: #374151; font-family: 'Inter', -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif;">
        Lignes {{ $poleActivites->firstItem() }} à {{ $poleActivites->lastItem() }} sur {{ $poleActivites->total() }}
    </div>
    <div>
        {{ $poleActivites->links() }}
    </div>
