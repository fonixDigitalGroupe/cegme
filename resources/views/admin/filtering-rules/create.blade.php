...
@section('title', 'Créer une règle de filtrage')

@section('content')
<div style="margin-bottom: 1.5rem;">
    <h1 style="font-size: 1.5rem; font-weight: 600; color: #1a1a1a; margin: 0;">Créer une règle de filtrage</h1>
</div>
...
<form method="POST" action="{{ route('admin.filtering-rules.store') }}" style="background-color: #ffffff; padding: 1.5rem; border: 1px solid #d1d5db; border-radius: 4px;">
    @csrf
    ...
    <div style="margin-bottom: 1.5rem;">
        ...
        <select id="market_type" name="market_type" style="width: 100%; padding: 0.5rem 0.75rem; border: 1px solid #d1d5db; border-radius: 4px; font-size: 0.875rem;">
            ...
            <option value="bureau_d_etude" {{ old('market_type') === 'bureau_d_etude' ? 'selected' : '' }}>Bureau d'études</option>
            <option value="consultant_individuel" {{ old('market_type') === 'consultant_individuel' ? 'selected' : '' }}>Consultant individuel</option>
        </select>
    </div>
    ...
    <div style="margin-bottom: 1.5rem;">
        ...
        @foreach($activityPoles as $pole)
            <label style="display: flex; align-items: center; margin-bottom: 0.5rem; cursor: pointer;">
                <input type="checkbox" name="activity_pole_ids[]" value="{{ $pole->id }}" style="margin-right: 0.5rem;">
                <span>{{ $pole->name }} ({{ $pole->keywords->count() }} mots-clés)</span>
            </label>
        @endforeach
        @if($activityPoles->isEmpty())
            <p style="color: #6b7280; font-size: 0.875rem;">Aucun pôle d'activité disponible. <a href="{{ route('admin.activity-poles.create') }}" style="color: #3b82f6;">Créer un pôle d'activité</a></p>
        @endif
    </div>
