...
@section('title', 'Créer une source de filtrage')

@section('content')
<div style="max-width: 600px; margin: 0 auto;">
    <div style="margin-bottom: 1.5rem; text-align: center;">
        <h1 style="font-size: 1.5rem; font-weight: 600; color: #1a1a1a; margin: 0;">Créer une source de filtrage</h1>
    </div>
...
<form method="POST" action="{{ route('admin.filtering-rules.store') }}" style="background-color: #ffffff; padding: 1.5rem; border: 1px solid #d1d5db; border-radius: 4px;">
    ...
    <div style="margin-bottom: 1.5rem; max-width: 500px;">
        ...
        <input type="text" id="name" name="name" value="{{ old('name') }}" required style="width: 100%; padding: 0.625rem 0.875rem; border: 1px solid #d1d5db; border-radius: 6px; font-size: 0.875rem; transition: border-color 0.2s;" onfocus="this.style.borderColor='#3b82f6'" onblur="this.style.borderColor='#d1d5db'">
    </div>
    ...
    <div style="margin-bottom: 2rem; display: flex; align-items: center; gap: 1rem;">
        <label style="display: flex; align-items: center; cursor: pointer; padding: 0.5rem 0;">
            <input type="checkbox" name="is_active" value="1" checked style="margin-right: 0.75rem; width: 18px; height: 18px; accent-color: #10b981;">
